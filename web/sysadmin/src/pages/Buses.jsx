...
const Buses = () => {
  ...
  const fetchBuses = async () => {
    try {
      setLoading(true);
      const response = await axios.get(`http://localhost:8080/admin/buses?page=${page}&page_size=20`);
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:8080/admin/buses/${id}`);
      fetchBuses();
    } catch (error) {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-gray-900 mb-6">Buses</h1>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Route Up</th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Route Down</th>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {buses.map((bus) => (
                  <tr key={bus.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm font-medium text-gray-900">{bus.registration_number}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{bus.route_id_up || '-'}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{bus.route_id_down || '-'}</td>
                    <td className="px-6 py-4 text-sm text-gray-600">{bus.owner_id || '-'}</td>
